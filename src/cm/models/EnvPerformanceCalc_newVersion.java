 ...
 */
public final class EnvPerformanceCalc_newVersion {
    //Initialization
    //Conversion Factor
    double ConvF;
    //Weights
...
//    double TotalWater_TSP_SubScore;
    ...
    Layer layer_temp;
    Mix mix_temp;
    public void EnvAnalysisCalc() throws ParseException {
        ...
        for (String key_i:design_keys) {
            ...
            for (int index_j = 0; index_j < design_temp.getNumberOfLayers(); index_j++) {
                ...
                mix_temp = getAnAveragedMix(layer_temp.getMixes());
                // Environmental part
...
//                    mix_temp.setTW_Transportation_SubScore(TotalWater_TSP_SubScore);
                ...
                layer_temp.setAveraged_Mix(mix_temp);
    ...
    public Mix getAnAveragedMix (List<Mix> mixList) throws ParseException {
        Mix averagedMix = new Mix();
        int size = mixList.size();
        Double sum_GWP = 0.0;
        Double sum_ODP = 0.0;
        Double sum_AP = 0.0;
        Double sum_EP = 0.0;
        Double sum_POCP = 0.0;
        Double sum_DNEC = 0.0; // depletion of non-renewable energy consumption
        for (int i = 0; i< size;i ++){
            if (mixList.get(i).getUnit().equalsIgnoreCase("m3")){
                sum_GWP += mixList.get(i).getGWP();
                sum_ODP += mixList.get(i).getODP();
                sum_AP += mixList.get(i).getAP();
                sum_EP += mixList.get(i).getEP();
                sum_POCP += mixList.get(i).getPOCP();
            }else{
                sum_GWP += mixList.get(i).getGWP()*1.30795;
                sum_ODP += mixList.get(i).getODP()*1.30795;
                sum_AP += mixList.get(i).getAP()*1.30795;
                sum_EP += mixList.get(i).getEP()*1.30795;
                sum_POCP += mixList.get(i).getPOCP()*1.30795;
            }
        ...
        averagedMix.setAP(sum_AP/size);
        averagedMix.setEP(sum_EP/size);
        averagedMix.setPOCP(sum_POCP/size);
        averagedMix.setTotalPrimaryEnergyConsumption(String.valueOf(sum_DNEC/size));
        return averagedMix;
    }
}
